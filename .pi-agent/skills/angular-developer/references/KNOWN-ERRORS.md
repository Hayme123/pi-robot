...
### ERR-038 — Orders Card Missing from Standalone Imports

**Symptom:** Angular reported `ntv-card` as an unknown element and rejected its `fullWidth` input in the orders template.

**Root Cause:** The orders standalone component used `ntv-card` in its template but imported only the toolbar and table Pantry components.

**Verified Fix:** Import `Card` from `@ntv360/component-pantry` and add `Card` to the orders component `imports` array.

**Rule:** Every Pantry component used by a standalone child template must be imported and registered in that same component.

## ERR-039 — Literal Escape Sequences in Tailwind Config

**Symptom:** Angular Sass reported `Unexpected token, expected "("` while compiling global styles.

**Root Cause:** A targeted edit inserted literal `\\n` escape sequences into the Tailwind `colors` object instead of actual newlines, making the configuration invalid for the Sass/Tailwind pipeline.

**Verified Fix:** Replaced the literal escape sequences with real line breaks in `tailwind.config.js`.

**Rule:** When editing JavaScript config files, verify newline characters are actual source line breaks before rebuilding.

## ERR-040 — Tailwind Color Token with CamelCase Key

**Symptom:** Angular Sass reported that `text-big-stone-900` did not exist.

**Root Cause:** The Tailwind configuration uses the camelCase token `bigStone`, so the generated utility name is not the hyphenated `big-stone` form.

**Verified Fix:** Use the existing semantic `text-ink` token for the branch row text instead of guessing a generated utility name.

**Rule:** Confirm the exact Tailwind utility generated by the configured token key; prefer existing semantic tokens when a matching visual color is already available.
